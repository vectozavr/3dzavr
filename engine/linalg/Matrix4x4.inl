...
inline Matrix4x4 Matrix4x4::View(const Matrix4x4 &transformMatrix) {
    ...
    if(transformMatrix.arePerpendicularAxis()) {
...
inline Matrix4x4 Matrix4x4::inverse() const {
    ...
    inv._arr[0][0] =  ( m[1][1] * A2323 - m[1][2] * A1323 + m[1][3] * A1223 ) / det;
    inv._arr[0][1] = -( m[0][1] * A2323 - m[0][2] * A1323 + m[0][3] * A1223 ) / det;
    inv._arr[0][2] =  ( m[0][1] * A2313 - m[0][2] * A1313 + m[0][3] * A1213 ) / det;
    inv._arr[0][3] = -( m[0][1] * A2312 - m[0][2] * A1312 + m[0][3] * A1212 ) / det;
    inv._arr[1][0] = -( m[1][0] * A2323 - m[1][2] * A0323 + m[1][3] * A0223 ) / det;
    inv._arr[1][1] =  ( m[0][0] * A2323 - m[0][2] * A0323 + m[0][3] * A0223 ) / det;
    inv._arr[1][2] = -( m[0][0] * A2313 - m[0][2] * A0313 + m[0][3] * A0213 ) / det;
    inv._arr[1][3] =  ( m[0][0] * A2312 - m[0][2] * A0312 + m[0][3] * A0212 ) / det;
    inv._arr[2][0] =  ( m[1][0] * A1323 - m[1][1] * A0323 + m[1][3] * A0123 ) / det;
    inv._arr[2][1] = -( m[0][0] * A1323 - m[0][1] * A0323 + m[0][3] * A0123 ) / det;
    inv._arr[2][2] =  ( m[0][0] * A1313 - m[0][1] * A0313 + m[0][3] * A0113 ) / det;
    inv._arr[2][3] = -( m[0][0] * A1312 - m[0][1] * A0312 + m[0][3] * A0112 ) / det;
    inv._arr[3][0] = -( m[1][0] * A1223 - m[1][1] * A0223 + m[1][2] * A0123 ) / det;
    inv._arr[3][1] =  ( m[0][0] * A1223 - m[0][1] * A0223 + m[0][2] * A0123 ) / det;
    inv._arr[3][2] = -( m[0][0] * A1213 - m[0][1] * A0213 + m[0][2] * A0113 ) / det;
    inv._arr[3][3] =  ( m[0][0] * A1212 - m[0][1] * A0212 + m[0][2] * A0112 ) / det;

    return inv;
}

inline bool Matrix4x4::arePerpendicularAxis() const {
    /*
     * left   = x();
     * up     = y();
     * lookAt = z();
    ...
    if((std::abs(x().dot(y())) < Consts::EPS) &&
       (std::abs(x().dot(z())) < Consts::EPS) &&
       (std::abs(y().dot(z())) < Consts::EPS)) {
        // When left, up and lookAt are perpendicular to each other
        return true;
    }
