...
World::World(const std::string &filename, double scale) {
    ...
    Log::log("World::World(): loaded map from " + filename + "' with " + std::to_string(objects["map"].data().size()) + " triangles.");
}

void World::addMesh(Mesh &mesh, const string &name) {
    objects.emplace(name, std::move(mesh));
    Log::log("World::addMesh(): inserted lvalue mesh '" + name + "' with " + std::to_string(objects[name].data().size()) + " triangles.");

}

void World::addMesh(const Mesh& mesh, const string &name) {
    objects.emplace(name, mesh);
    Log::log("World::addMesh(): inserted rvalue mesh '" + name + "' with " + std::to_string(objects[name].data().size()) + " triangles.");
}
...
void World::loadObj(const string &filename, const string &name, double scale) {
    ...
    Log::log("World::loadObj(): inserted mesh from " + filename + " with name '" + name + "' with " + std::to_string(objects[name].data().size()) + " triangles.");
}
